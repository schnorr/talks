...
* Demonstração
** Demonstração de acesso ao PCAD

#+attr_latex: :width .8\linewidth
[[./img/terminal.png]]

* Pesquisa
** Pesquisas do GPPD-HPC (https://www.inf.ufrgs.br/gppd/site/)
*** Três professores
- Philippe Olivier Alexandre Navaux
- Arthur Francisco Lorenzon
- Lucas Mello Schnorr
*** Tópicos transversais do grupo

|------------------------+--------------------------------------------------------------|
| Programação paralela   | OpenMP, MPI, CUDA, modelos task-based (StarPU)               |
| Escalonamento          | Balanceamento de carga, mapeamento de tarefas/dados, DCT     |
| Eficiência energética  | Green computing, EDP, DVFS, gestão de frequência em exascale |
| Infraestrutura HPC     | Gestão de clusters (Slurm, BCM), PCAD, reprodutibilidade     |
| Aplicações científicas | Sísmica, CFD, stencils geofísicos, grafos, deep learning     |
| HPC + IA               | Treinamento e inferência de LLMs, GPU sharing, AI Factories  |
| Visualização           | Traces de performance, ferramentas PajeNG, StarVZ            |
| Simulação              | Simulação de sistemas computacionais (SimGrid)               |
|------------------------+--------------------------------------------------------------|

** Pesquisa: observação do comportamento de aplicações task-based 1/2

Fatoração Cholesky por blocos e o seu Directed-Acyclic Graph (para $N=5$).
...
** Pesquisa: observação do comportamento de aplicações task-based 2/2

StarPU /runtime/ gera um rastro (/trace/) de um escalonamento do DAG nos recursos.

R/StarVZ https://cran.r-project.org/web/packages/starvz/ para análise.
...
- Como melhorar o escalonamento do DAG?
- Dado uma aplicação, como chegar a um algoritmo por blocos?
- Como participar o DAG em sistemas heterogêneos em nível de sistema?
- Como fazer uma visualização de dados de grande escala?
